...
start
==
###### pwnable.tw의 start문제입니다.
  
Overview
--
start 파일을 다운로드 받아서 실행하거나,  
chall.pwnable.tw:10000에 접속하면 다음과 같은 실행화면이 나옵니다.  
```
...
입력값을 길게 주면 Segmentation fault가 나고 비정상 종료하니깐 입력값을 잘 만들어서 리턴 주소를 덮는 문제인가? 생각을 하고 넘어갔어요.  
```
lucy@ubuntu:~/pwnable.tw/start$ ./start
...
checksec으로 보호기법을 확인해보면 다음과 같이 나옵니다.  
아무것도 없어요!
```
...
main 어셈블리가 안나와서 일단 start로 실행해봤어요.  
...
입력받는 부분 보려고 read함수를 호출하는지 확인해봤는데요! 또 없어요.
```
pwndbg> disas read
No symbol table is loaded.  Use the "file" command.
```
...
`_start`함수의 어셈블리를 보면 syscall을 이용해서 write를 호출한 후, read함수를 호출합니다.  
```
pwndbg> disas _start
...
이때, return_addr를 알기 위해서는 esp값을 알아야 합니다. esp값을 leak해서 shellcode 주소를 return_addr에 넣어주어야 하는데요.    
